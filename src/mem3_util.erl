...
-export([hash/1, name_shard/2, create_partition_map/5, build_shards/2,
    n_val/2, to_atom/1, to_integer/1, open_db_doc/1, write_db_doc/1,
    delete_db_doc/1, ensure_exists/1, load_shards_from_disk/1,
...
load_shards_from_disk(DocId) when is_binary(DocId) ->
    DbName = ?l2b(couch_config:get("mem3", "shard_db", "dbs")),
    {ok, Db} = couch_db:open(DbName, []),
    try load_shards_from_db(Db, DocId) after couch_db:close(Db) end.
